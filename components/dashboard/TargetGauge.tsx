...
export default function TargetGauge({ leads, targets, metricMode }: TargetGaugeProps) {
  ...
  return (
    <div className="bg-white rounded-[14px] border border-[#EBEBE7] p-6
      ...
      <div className="relative w-[208px] h-[208px] flex-shrink-0 mx-auto md:mx-0">
        <svg width="208" height="208" viewBox="0 0 208 208">
          ...
          <circle cx="104" cy="104" r={R}
            fill="none" stroke="#F5F5F2" strokeWidth="14" />
          {/* Progress arc */}
          <circle cx="104" cy="104" r={R}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#A0A09A] mt-1">
            YTD vs Target
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0 w-full">
        {/* Eyebrow */}
        <p className="text-[10.5px] font-bold uppercase tracking-[0.12em] text-[#065F46]">
          Target {metricMode === 'netto' ? 'Netto' : 'Bruto'} 2026
        </p>

        {/* Big number */}
        <div className="flex items-baseline gap-2 mt-1">
          <span className="text-[32px] font-bold tracking-[-0.015em] text-[#1A1A18] leading-tight">
            {formatRupiahShort(ytdClosing)}
          </span>
          <span className="text-[#A0A09A] font-medium text-sm">
            / {formatRupiahShort(totalTarget)}
          </span>
        </div>

        {/* Insight sentence */}
        <p className="text-[13px] text-[#6B6B65] mt-1.5 leading-normal">
          ...
          {QUARTERS.map(q => {
            ...
            return (
              ...
              >
                ...
                {/* Closing value */}
                <p className="text-[13px] font-semibold mb-0.5 text-[#1A1A18]">
                  {closing > 0 ? formatRupiahShort(closing) : '—'}
                </p>
                <p className="text-[10px] text-[#A0A09A] mb-1.5">
                  dari {formatRupiahShort(targetVal)}
                </p>

                {/* Progress bar */}
                <div className="h-[3px] bg-[#F5F5F2] rounded-sm overflow-hidden">
                  <div style={{
                    height: '100%',
